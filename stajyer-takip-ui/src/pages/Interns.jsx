...
export default function Interns() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        const data = await internsApi.getAll({
          page, pageSize,
          search, status,
          sortBy, sortDir
        });
        ...
        setTotal(Number(data?.totalCount ?? items.length ?? 0));
      } catch (e) {
        setError(e?.response?.data?.message || e.message || "Veri alınamadı");
      } finally {
        setLoading(false);
      }
    };
            ...
            {loading ? (
              ...
              <tr key={i.id} style={{ borderTop: "1px solid #f1f1f1" }}>
                ...
                <td style={{ padding: 8, textAlign: "right", whiteSpace: "nowrap" }}>
                  <button type="button" onClick={() => nav(`/interns/${i.id}/edit`)} style={{ padding: "6px 10px", borderRadius: 8, border: "1px solid #ccc", marginRight: 6 }}>
                    Düzenle
                  </button>
                  <button
                    type="button"
